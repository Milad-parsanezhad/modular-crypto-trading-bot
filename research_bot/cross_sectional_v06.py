...
def _month_range(start_month:str,end_month:str|None=None):
    ...
    end_month=end_month or (now.to_period('M')-1).strftime('%Y-%m')
...
def fetch_um_monthly_raw_klines(symbol:str,interval:str='8h',start_month:str='2022-01',end_month:str|None=None,verify_checksum:bool=True,kind:str='klines'):
    ...
    for per in _month_range(start_month,end_month):
        ...
        raw['timestamp_open']=_utc(raw['open_time']); raw['timestamp']=_utc(raw['close_time'])
...
def fetch_um_monthly_funding(symbol:str,start_month:str='2022-01',end_month:str|None=None,verify_checksum:bool=True):
    ...
    for per in _month_range(start_month,end_month):
        ...
        raw['timestamp']=_utc(raw['calc_time']); raw['funding_rate']=pd.to_numeric(raw[rate_col],errors='coerce')
...
def build_symbol_panel(symbol:str,start_month:str,end_month:str|None=None,verify_checksum:bool=True):
    ...
    x['symbol']=symbol.upper(); x=x.sort_values('timestamp')
    if not prem.empty:
        pp=prem[['timestamp','close']].rename(columns={'close':'premium_index_close'}).sort_values('timestamp'); x=pd.merge_asof(x,pp,on='timestamp',direction='backward',tolerance=pd.Timedelta('8h'))
    if not fund.empty:
        x=pd.merge_asof(x,fund.sort_values('timestamp'),on='timestamp',direction='backward',tolerance=pd.Timedelta('8h'))
    return x,{'price':pm,'premium':prm,'funding':fm}


def add_cross_sectional_features(panel:pd.DataFrame):
    x=panel.copy(); x['timestamp']=pd.to_datetime(x.timestamp,utc=True); x=x.sort_values(['symbol','timestamp']).reset_index(drop=True)
...
def _model(kind:str,seed:int):
    if kind=='ridge_logit':
        return Pipeline([('imputer',SimpleImputer(strategy='median')),('scale',StandardScaler()),('model',LogisticRegression(C=0.25,penalty='l2',solver='lbfgs',max_iter=1500,random_state=seed))])
    if kind=='hgb':
