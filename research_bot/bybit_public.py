...
BYBIT_INTERVALS = {
    ...
    "12hour": "720", "1day": "D", "1week": "W",
}
PERIOD_MS = {
    "1min": 60_000, "3min": 180_000, "5min": 300_000, "15min": 900_000,
    "30min": 1_800_000, "1hour": 3_600_000, "2hour": 7_200_000,
    "4hour": 14_400_000, "6hour": 21_600_000, "12hour": 43_200_000,
...
def fetch_bybit_spot_ohlcv(
    ...
    limit: int = 1000,
    max_pages: int = 20,
    request_fn: Callable[[str, dict, int], dict] = _get_json,
) -> pd.DataFrame:
    """Fetch Bybit public spot klines using deterministic backward pagination.

    Intended for external-venue research replication. Only exchange OHLCV/turnover
    fields are returned; no order-flow variable is inferred or fabricated. Bybit
    returns klines newest-first, which are normalized to ascending UTC timestamps.
    """
    ...
    for _ in range(max_pages):
        payload = request_fn(
            "/v5/market/kline",
            {
                "category": "spot", "symbol": _symbol(symbol),
                "interval": BYBIT_INTERVALS[period], "start": target_start,
                "end": end_cursor, "limit": min(int(limit), 1000),
            },
            20,
        )
        if payload.get("retCode") != 0:
            raise RuntimeError(f"Bybit API error for spot kline: {payload}")
    ...
    df["market"] = _symbol(symbol); df["source"] = "bybit_public_spot"
...
def audit_bybit_ohlcv(df: pd.DataFrame, period: str) -> dict:
    if period not in PERIOD_MS:
        raise ValueError(f"Unsupported Bybit period: {period}")
    if df.empty:
        return {"rows": 0, "gap_count": 0, "estimated_missing_bars": 0, "coverage_start": None, "coverage_end": None}
    ts = pd.to_datetime(df["timestamp"], utc=True).sort_values().drop_duplicates()
    step = pd.Timedelta(milliseconds=PERIOD_MS[period])
    delta = ts.diff().dropna(); gaps = delta > step * 1.5
    missing = int(sum(max(0, round(d / step) - 1) for d in delta[gaps]))
    return {
        "rows": int(len(ts)), "gap_count": int(gaps.sum()), "estimated_missing_bars": missing,
        "coverage_start": ts.iloc[0].isoformat(), "coverage_end": ts.iloc[-1].isoformat(),
        "median_spacing_seconds": float(delta.median().total_seconds()) if len(delta) else None,
    }


def fetch_bybit_open_interest_history(
    symbol: str = "BTCUSDT", interval: str = "4h", start_ms: int | None = None,
    end_ms: int | None = None, limit: int = 200, max_pages: int = 150,
) -> pd.DataFrame:
    ...
    for _ in range(max_pages):
        payload = _get_json(
            ...
            {"category": "linear", "symbol": symbol.upper(), "intervalTime": interval,
             "startTime": start_ms, "endTime": end_ms, "limit": min(int(limit), 200), "cursor": cursor},
            20,
        )
        result = payload.get("result") or {}; data = result.get("list") or []
        for row in data:
            key = str(row.get("timestamp"))
            if key not in seen:
                seen.add(key); rows.append(row)
        next_cursor = result.get("nextPageCursor") or ""
        if not data or not next_cursor or next_cursor == cursor: break
        cursor = next_cursor; time.sleep(0.03)
    if not rows:
    ...
    df["single_open_interest"] = pd.to_numeric(df["singleOpenInterest"], errors="coerce") if "singleOpenInterest" in df.columns else pd.NA
