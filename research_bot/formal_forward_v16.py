...
def build_formal_forward_evaluation(snapshots: Iterable[dict], *, generated_at: datetime | None = None) -> dict:
    ...
    v0.16 is a reporting/evaluation layer. It never changes v0.15 thresholds and
    never promotes LIVE execution. Formal risk ratios are exposed only after the
    pre-registered v0.15 sample gate passes and enough independent snapshot
    returns exist to make the ratio numerically meaningful.
    """
    generated_at = generated_at or datetime.now(timezone.utc)
    rows = _dedupe_sort(snapshots)
    if not rows:
    ...
    safety_passes = [bool((s.get("safety_contract") or {}).get("ok")) for s in rows]
    modes = [str((s.get("service") or {}).get("execution_mode") or "UNKNOWN") for s in rows]
    promotions = [str((s.get("scientific_state") or {}).get("live_promotion") or "UNKNOWN") for s in rows]
    ...
    observed_dds = [
        x for x in (_f((s.get("paper_account") or {}).get("current_drawdown")) for s in rows)
        if x is not None
    ]

    first_ts = _dt(first.get("captured_at"))
    last_ts = _dt(latest.get("captured_at"))
    ...
    positions_by_symbol = {
        str(x.get("symbol")): {
            "quantity": _f(x.get("quantity"), 0.0),
            "avg_price": _f(x.get("avg_price"), 0.0),
        }
        for x in latest_positions
        if isinstance(x, dict) and x.get("symbol")
    }

    evidence_states = Counter(str((s.get("scientific_state") or {}).get("evidence_state") or "UNKNOWN") for s in rows)
    ...
    return {
        ...
        "paper_performance": {
            ...
            "annualization_factor": ratios["annualization_factor"],
        ...
        "timeline": [
            ...
            for s, (ts, eq) in zip([x for x in rows if _f((x.get("paper_account") or {}).get("equity")) not in (None, 0.0)], series)
        ],
        "warnings": [
...
def chapter4_markdown(evaluation: dict) -> str:
    ...
    lines = [
        ...
        "",
        f"- Snapshot count: `{c['snapshot_count']}`",
        f"- Snapshot window: `{c['snapshot_window_hours']:.2f} h`",
