...
def independent_oi_confirmation(start_month:str,end_month:str|None,fee_bps:float,slippage_bps:float):
    # Frozen discovery definition from v0.5: 4h, Bear regime, +OI vs baseline. Different asset: ETHUSDT.
    ...
    feat,fams=build_point_in_time_features(frame)
    feat=add_point_in_time_regimes(feat)
    ...
    return {
        'asset':'ETHUSDT','frozen_hypothesis':'OI × Bear regime × 4h',
        'summary':summ.to_dict('records'),'bear_incremental_comparison':bear.to_dict('records'),
        'robustness':robust,'kline_meta':meta,'oi_meta':oimeta,
        'note':'Independent asset confirmation only; same venue family and methodology as discovery.'
    }


def main():
    p=argparse.ArgumentParser()
    p.add_argument('--start-month',default='2022-01')
    p.add_argument('--end-month',default=None)
    p.add_argument('--output-dir',default='artifacts/v06')
    ...
    for s in symbols:
        ...
        except Exception as e: failures[s]=f'{type(e).__name__}: {e}'
    if len(panels)<6: raise RuntimeError(f'Need >=6 usable perpetuals, got {len(panels)}; failures={failures}')
    panel=pd.concat(panels,ignore_index=True).sort_values(['timestamp','symbol'])
    panel.to_csv(out/'cross_sectional_raw_panel.csv',index=False)
    ...
    report={
      'research_status':'v0.6_cross_sectional_perpetuals_independent_confirmation_not_production',
      'universe_requested':symbols,'universe_usable':sorted(panel.symbol.unique().tolist()),'archive_failures':failures,
      'coverage':{'start':panel.timestamp.min().isoformat(),'end':panel.timestamp.max().isoformat(),'rows':int(len(panel)),'settlements':int(panel.timestamp.nunique())},
      'cross_sectional_design':{
        'clock':'completed 8h perpetual bars aligned to settlement availability',
        'variants':['baseline market/liquidity/order-flow','baseline + funding/premium crowding'],
        'models':['ridge logistic','HistGradientBoosting'],'portfolio':'dollar-neutral top/bottom 20% cross-sectional score','one_way_cost_bps':a.fee_bps+a.slippage_bps,
        'labels':['8h forward return','24h forward return'],'validation':'expanding chronological folds; no test tuning'
      },
      'cross_sectional_summary':summary.to_dict('records'),'oi_independent_confirmation':oi,'archive_meta':archive_meta,
      'warnings':['Cross-sectional v0.6 is a research replication, not a live strategy.','The first panel uses 12 liquid perpetual candidates and will only scale to 19-30 after data-quality validation.','No deep model is allowed to replace simple baselines until incremental information is demonstrated.']
    }
    (out/'v06_report.json').write_text(json.dumps(clean(report),indent=2),encoding='utf-8')
    print('===V06_REPORT==='); print(json.dumps(clean(report),indent=2)); print('===END_V06_REPORT===')
