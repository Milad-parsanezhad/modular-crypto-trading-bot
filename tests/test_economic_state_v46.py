...
def test_timeout_sign_split_is_exactly_zero_boundary() -> None:
    frame = pd.DataFrame({
        "outcome": ["TARGET", "STOP", "TIME", "TIME", "TIME"],
        "net_r": [2.7, -1.1, 0.4, 0.0, -0.2],
    })
    assert encode_state_v46(frame, R0).tolist() == ["TARGET", "STOP", "TIME", "TIME", "TIME"]
    assert encode_state_v46(frame, R1).tolist() == [
        "TARGET", "STOP", "TIME_POSITIVE", "TIME_NONPOSITIVE", "TIME_NONPOSITIVE"
    ]
...
def test_expected_r_uses_only_supplied_state_means() -> None:
    p = pd.DataFrame({
        "p_target_v46": [0.2],
        "p_stop_v46": [0.3],
        "p_time_positive_v46": [0.4],
        "p_time_nonpositive_v46": [0.1],
    })
...
def test_perfect_multiclass_probabilities_have_zero_brier() -> None:
...
def test_reliability_resolution_is_finite_and_nonnegative() -> None:
